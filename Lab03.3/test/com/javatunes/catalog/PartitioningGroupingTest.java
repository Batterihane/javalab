...
public class PartitioningGroupingTest {
  ...
  public void testPartitionByPrice() {
    Map<Boolean, List<MusicItem>> collect = allMusicItems.stream()
            .collect(partitioningBy(item -> item.getPrice() > 14));
    dumpMap(collect);
  ...
  public void testPartitionByMusicCategoryRock() {
    Map<Boolean, List<MusicItem>> collect = allMusicItems.stream()
            .collect(partitioningBy(item -> item.getMusicCategory() == MusicCategory.ROCK || item.getMusicCategory() == MusicCategory.CLASSIC_ROCK));
    dumpMap(collect);
  }
  ...
  public void testGroupByMusicCategory() {
    Map<MusicCategory, List<MusicItem>> collect = allMusicItems.stream()
            .collect(groupingBy(MusicItem::getMusicCategory));
    collect.keySet()
            .forEach(category -> collect.get(category)
                    .forEach(item -> assertSame(item.getMusicCategory(), category)));
  }
  ...
  public void testGroupByPrice() {
    Map<Double, List<MusicItem>> collect = allMusicItems.stream()
            .collect(groupingBy(MusicItem::getPrice));
    collect.keySet()
            .forEach(price -> collect.get(price)
                    .forEach(item -> assertEquals(item.getPrice(), price)));
  }
